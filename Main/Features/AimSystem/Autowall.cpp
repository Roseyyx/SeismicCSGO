...
{
	const Vector vecPosition = pLocal->GetEyePos();
	...
	//Console::Print("Autowall Info", "GetDamage Call | Damage Dealt: {}", data.flCurrentDamage);
...
void CAutoWall::ScaleDamage(const int iHitGroup, CBaseEntity* pEntity, const float flWeaponArmorRatio, const float flWeaponHeadShotMultiplier, float& flDamage) {
	...
	static CConVar* mp_damage_scale_ct_head = Interfaces::Convar->FindVar("mp_damage_scale_ct_head");
	static CConVar* mp_damage_scale_t_head = Interfaces::Convar->FindVar("mp_damage_scale_t_head");

	static CConVar* mp_damage_scale_ct_body = Interfaces::Convar->FindVar("mp_damage_scale_ct_body");
	static CConVar* mp_damage_scale_t_body = Interfaces::Convar->FindVar("mp_damage_scale_t_body");
...
void CAutoWall::ClipTraceToPlayers(const Vector& vecAbsStart, const Vector& vecAbsEnd, const unsigned int fMask, ITraceFilter* pFilter, Trace_t* pTrace, const float flMinRange)
{
	...
	for (int i = 1; i < Interfaces::Globals->nMaxClients; i++)
	{
		...
		Interfaces::EngineTrace->ClipRayToEntity(ray, fMask | CONTENTS_HITBOX, pEntity, &trace);
...
bool CAutoWall::TraceToExit(Trace_t& enterTrace, Trace_t& exitTrace, const Vector& vecPosition, const Vector& vecDirection, const CBaseEntity* pClipPlayer)
{
	...
	while (flDistance <= 90.0f)
	{
		...
		if (!iStartContents)
			iStartContents = Interfaces::EngineTrace->GetPointContents(vecStart, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);

		const int iCurrentContents = Interfaces::EngineTrace->GetPointContents(vecStart, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);
		...
		if (!(iCurrentContents & MASK_SHOT_HULL) || (iCurrentContents & CONTENTS_HITBOX && iCurrentContents != iStartContents))
		{
			...
			Interfaces::EngineTrace->TraceRay(rayWorld, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr, &exitTrace);

			if (static CConVar* sv_clip_penetration_traces_to_players = Interfaces::Convar->FindVar("sv_clip_penetration_traces_to_players"); sv_clip_penetration_traces_to_players != nullptr && sv_clip_penetration_traces_to_players->GetBool())
			{
			...
			if (exitTrace.bStartSolid && exitTrace.surface.uFlags & SURF_HITBOX)
			{
				...
				Interfaces::EngineTrace->TraceRay(ray, MASK_SHOT_HULL, &filter, &exitTrace);
...
bool CAutoWall::HandleBulletPenetration(CBaseEntity* pLocal, const CCSWeaponData* pWeaponData, const surfacedata_t* pEnterSurfaceData, FireBulletData_t& data)
{
	...
	static CConVar* ff_damage_reduction_bullets = Interfaces::Convar->FindVar("ff_damage_reduction_bullets");
	static CConVar* ff_damage_bullet_penetration = Interfaces::Convar->FindVar("ff_damage_bullet_penetration");
	...
	Trace_t exitTrace = { };
	if (!TraceToExit(data.enterTrace, exitTrace, data.enterTrace.vecEnd, data.vecDirection, pLocal) && !(Interfaces::EngineTrace->GetPointContents(data.enterTrace.vecEnd, MASK_SHOT_HULL, nullptr) & MASK_SHOT_HULL))
		return false;

	const surfacedata_t* pExitSurfaceData = Interfaces::PhysicsProps->GetSurfaceData(exitTrace.surface.nSurfaceProps);
...
bool CAutoWall::SimulateFireBullet(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, FireBulletData_t& data)
{
	...
	CCSWeaponData* pWeaponData = Interfaces::WeaponSystem->GetWeaponData(pWeapon->GetItemDefinitionIndex());
	...
	while (data.iPenetrateCount > 0 && data.flCurrentDamage >= 1.0f)
	{
		...
		Interfaces::EngineTrace->TraceRay(ray, MASK_SHOT_HULL | CONTENTS_HITBOX, &filter, &data.enterTrace);
		...
		const surfacedata_t* pEnterSurfaceData = Interfaces::PhysicsProps->GetSurfaceData(data.enterTrace.surface.nSurfaceProps);
		const float flEnterPenetrationModifier = pEnterSurfaceData->game.flPenetrationModifier;
