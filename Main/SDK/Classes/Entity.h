...
public:
	...
	NetvarVariable(Vector, GetOrigin, "CBaseEntity->m_vecOrigin");
	NetvarVariable(int, GetCollisionGroup, "CBaseEntity->m_CollisionGroup");

	NetvarVariable(int, GetArmor, "CCSPlayer->m_ArmorValue");
	NetvarVariable(Vector, GetEyeAngles, "CCSPlayer->m_angEyeAngles");
	NetvarVariable(bool, HasHelmet, "CCSPlayer->m_bHasHelmet");
	NetvarVariable(bool, HasHeavyArmor, "CCSPlayer->m_bHasHeavyArmor");
	NetvarVariable(int, GetSurvivalTeam, "CCSPlayer->m_nSurvivalTeam");

	NetvarVariable(Vector, GetViewOffset, "CBasePlayer->m_vecViewOffset[0]");
	NetvarVariable(float, GetSpread, "CBasePlayer->m_flSpread");
	NetvarVariable(float, GetInaccuracy, "CBasePlayer->m_flInaccuracy");

	NetvarVariable(CBaseHandle, GetActiveWeaponHandle, "CBaseCombatCharacter->m_hActiveWeapon");
	...
	int& GetTakeDamage()
	{
		static const std::uintptr_t uTakeDamageOffset = *reinterpret_cast<std::uintptr_t*>(Memory::FindPattern("client.dll", "80 BE ? ? ? ? ? 75 46 8B 86") + 0x2);
	...
	Vector					GetHitboxPosistion(const int iHitbox);
	bool					IsBreakable();
};
...
{
public:
	NetvarVariable(float, GetNextPrimaryAttack, "CBaseCombatWeapon->m_flNextPrimaryAttack");
	
	NetvarVariable(short, GetItemDefinitionIndex, "CBaseAttributableItem->m_iItemDefinitionIndex");
};
...
public:
	NetvarVariable(int, GetZoomLevel, "CWeaponCSBaseGun->m_zoomLevel");
};
...
{
public:
	NetvarVariable(bool, IsBroken, "CBreakableSurface->m_bIsBroken");
};