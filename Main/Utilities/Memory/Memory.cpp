...
std::uintptr_t Memory::FindPattern(const std::string_view szModuleName, const std::string_view szPattern)
{
	...
	if (hModule == nullptr)
		throw std::runtime_error(std::format("failed to get handle for: {}", szModuleName));
...
std::uintptr_t Memory::FindPattern(const std::uint8_t* uRegionStart, const std::uintptr_t uRegionSize, const std::string_view szPattern)
{
	...
	Console::Print("Pattern Error", "Pattern not found: [{}]", szPattern);
...
void* Memory::GetModuleBaseHandle(const std::string_view szModuleName)
{
	...
	Console::PushConsoleColor(FOREGROUND_INTENSE_RED);
	Console::Print("Module bASE Error", "Module Base not found: [{}]", szModuleName);
	Console::PopConsoleColor();
	return nullptr;
}
...
void* Memory::GetExportAddress(const void* pModuleBase, const std::string_view szProcedureName)
{
	...
	Console::PushConsoleColor(FOREGROUND_INTENSE_RED);
	Console::Print("Export Error", "Export not found: [{}]", szProcedureName);
	Console::PopConsoleColor();
	return nullptr;
}

std::vector<std::uintptr_t> Memory::GetCrossReferences(const std::uintptr_t uAddress, std::uintptr_t uRegionStart, const std::size_t uRegionSize)
{
...
bool Memory::GetSectionInfo(const std::uintptr_t uBaseAddress, const std::string_view szSectionName, std::uintptr_t* puSectionStart, std::uintptr_t* puSectionSize)
{
...
std::uintptr_t Memory::GetVTableTypeDescriptor(const std::string_view szModuleName, const std::string_view szTableName)
{
	...
	Console::PushConsoleColor(FOREGROUND_INTENSE_RED);
	Console::Print("VTable Error", "Virtual table type descriptor not found: [{}] [{}] ", szModuleName, szTableName);
	Console::PopConsoleColor();
	return 0U;
}

std::uintptr_t* Memory::GetVTablePointer(const std::string_view szModuleName, const std::string_view szTableName)
{
	...
	if (!GetSectionInfo(uBaseAddress, ".rdata", &uRDataStart, &uRDataSize))
		return nullptr;
	...
	for (const auto& uCrossReference : GetCrossReferences(uTypeDescriptor, uRDataStart, uRDataSize))
	{
		...
		if (!GetSectionInfo(uBaseAddress, ".text", &uTextStart, &uTextSize))
			return nullptr;
	...
	Console::PushConsoleColor(FOREGROUND_INTENSE_RED);
	Console::Print("Vtable Error", "Virtual table pointer not found: [{}] [{}] ", szModuleName, szTableName);
	Console::PopConsoleColor();
	return nullptr;
}

bool Memory::IsValidCodePtr(const void* pPointer)
{
	if (pPointer == nullptr)
		return false;
...
std::string Memory::BytesToPattern(const std::uint8_t* arrBytes, const std::size_t uSize)
{
